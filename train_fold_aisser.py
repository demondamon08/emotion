...
def run_fold(fold: int, config_path: str, include_zoom: bool, n_iter: int = 25, use_attn: bool = False):
    ...
    for i  in range(n_iter):
        ...
        if not include_zoom:
            ...
            open(f"{thaiser_module.experiment_dir}/confusion_matrix_zoom.txt", "a").write(f"{cm}")
...
def main(arguments):
    ...
    thaiser_module = ThaiSERDataModule(test_fold=6, **thaiser_config)
    thaiser_module.prepare_data()
    #thaiser_module._prepare_labels()
    print(arguments)
